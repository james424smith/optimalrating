      ...
      >
        ...
        <nuxt-link to="/" class="dropdown-toggle logo-toggle">
          <img
            ...
            alt="Optimal Rating Logo"
            style="height: 45px; margin-left: 10px; margin-right: auto; margin-right: 50px"
          />
        </nuxt-link>
        <!-- search form -->
        ...
        <div class="navbar-right">
          <ul class="nav navbar-nav">
            ...
            <li class="dropdown notifications-menu" v-if="user && user.pending_friends">
              <!--style="font-size: 16px;"-->
              <button
                class="dropdown-toggle drp-toggle"
              ...
              <ul
                :class="'dropdown-user-list dropdown-menu dropdown-menu-right '
                            +(toggleRequestFriends && user.pending_friends.length > 0 ? 'show': '')"
              >
                <li>
                  <h6 class="request-friend-title">{{$t('lbl.request_friend_notification')}}</h6>
                </li>
                <li
                  v-for="(pends, index) in user.pending_friends"
                  v-if="user.pending_friends.length > 0"
                >
                  <div class="notify-friend-list">
                    ...
                    <div class="friend-button">
                      <button
                        class="approve-friend btn-optimal"
                        @click="answerFriend(pends.user,index, 'yes')"
                      >
                        <i class="fa fa-check"></i>
                      </button>
                      <button
                        class="approve-friend btn-secondary"
                        @click="answerFriend(pends,index, 'no')"
                      >
            ...
            <li class="dropdown notifications-menu">
              <div
                class="imagebox-30 drp-toggle"
                data-toggle="dropdown"
                @click="loginDropDown"
                v-if="userLogin"
              >
                <img
                  v-if="user.user_details && user.user_details.profile_image"
              ...
              <ul v-else :class="'dropdown-menu dropdown-menu-right '+(toggleUser ? 'show': '')">
                <li>
                  <a href="#" @click="loginPopup">
                    <i class="mdi mdi-account-key"></i>
                    {{$t('lbl.login')}}
                  </a>
                  <a href="#" @click="registerPopup">
              ...
              <ul :class="'dropdown-menu dropdown-menu-right '+(toggleLang ? 'show': '')">
                <li v-for="country in countries">
                  <a href="#" class="countryLang" @click="changeLanguage(country.code)">
                    <img :src="cdn_url+'/images/country/'+country.flag" width="25" />
                    {{country.name}}
...
export default {
  ...
  methods: {
    answerFriend(pends, index, answer) {
      this.$store
        .dispatch("state_definition/requestAnswer", {
          id: pends.id,
          data: { answer: answer }
        })
        .then(response => {
          if (response.code !== 200) {
            this.$toast.error(this.$t('msg.friend_request_failed'));
          } else {
    ...
    visibleDropDown(e) {
      ...
      if (
        !dropdown_selector.is(e.target) &&
        dropdown_selector.has(e.target).length === 0
      ) {
